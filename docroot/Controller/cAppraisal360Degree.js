...
var dataComment = [];
var getCommentFn = function () {   // by DARIS -->  get data comment
...
var updateCommentFn = function () {   // by DARIS -->  update data comment
	$.ajax({
		...
		success: function (data) {
			...
			console.log("data:"+ JSON.stringify(dataChangeComment));
		}
	});
}
...
var assignTemplateCommentFn = function () { // by DARIS -->  gen template comment
	...
	htmlTemplateComment += "<div class='titlePanel'>Comment</div>";
	...
	htmlTemplateComment += "<div class=\"btn-group\">";
	htmlTemplateComment += "<button id='btnComment' style='border-bottom-right-radius: 0;   border-top-right-radius: 0;  min-width: 100%;' data-toggle=\"dropdown\" class=\"btn btn-success dropdown-toggle\"><span style='float:left;  margin-top: 3px;' class=\"icon-th-list\"></span><span id='span-filter' style='float:right;'> &nbsp;&nbsp;ทั้งหมด</span> </button>";
	htmlTemplateComment += "<ul style='min-width: 100%;' class=\"dropdown-menu\">";
	htmlTemplateComment += "<li><a onclick='btnCommentFn(3)'>ทั้งหมด</a></li>";
	htmlTemplateComment += "<li><a onclick='btnCommentFn(2)'>ตอบกลับแล้ว </a></li>";
	htmlTemplateComment += "<li><a onclick='btnCommentFn(1)'>ยังไม่ได้ตอบกลับ</a></li>";
	htmlTemplateComment += "</ul>";
	htmlTemplateComment += "</div>";

	htmlTemplateComment += "<div style=\"margin-left: 0px;\" class=\"btn-group\">";
	htmlTemplateComment += "<button style='border-bottom-left-radius: 0;   border-top-left-radius: 0;  min-width: 100%;' data-toggle=\"dropdown\" id='sel-empname' class=\"btn dropdown-toggle\"><span id='span-emp' style='float:left;'></span><span style='float:right;' class=\"caret\"></span></button>";
	htmlTemplateComment += " <ul id='SelComment' style='min-width: 100%;' class=\"dropdown-menu\">";

	var num = 1; var emp_id = '';
	$.each(dataComment['detail'], function (index, indexEntry) { // gen select 
		if (num) { num = 0; emp_id = indexEntry['emp_id']; }
		htmlTemplateComment += "<li onclick='detailCommentFn(\"" + indexEntry['emp_id'] + "\")'><a>" + indexEntry['emp_name'] + "</a></li>";
	});
	htmlTemplateComment += "</ul>";
	htmlTemplateComment += " </div>";
	...
	detailCommentFn(emp_id);
}

var btnCommentFn = function (num) { // by DARIS -->  change button
	var htmlSelect = "";
	var commentTXT = '';
	$("#SelComment").html('');   // clear select comment

	switch (num) {
		case 1: $("#span-filter").html(" &nbsp;&nbsp;ยังไม่ได้ตอบกลับ ");
			commentTXT = "no"; break;
		case 2: $("#span-filter").html(" &nbsp;&nbsp;ตอบกลับแล้ว ");
			commentTXT = "yes"; break;
		default: $("#span-filter").html(" &nbsp;&nbsp;ทั้งหมด ");
			commentTXT = "all";
	}
	...
	$.each(dataComment['detail'], function (index, indexEntry) {
		if (commentTXT == indexEntry['comment'] || commentTXT == 'all') {
			if (num) { num = 0; emp_id = indexEntry['emp_id']; }
			htmlSelect += "<li onclick='detailCommentFn(\"" + indexEntry['emp_id'] + "\")'><a>" + indexEntry['emp_name'] + "</a></li>";
		}
	});
	$("#SelComment").html(htmlSelect); // gen select comment
	detailCommentFn(emp_id);
};

var btnQualityFn = function (num) { // by DARIS -->  
	var htmlSelect = "";
	var commentTXT = '';
	$("#SelComment").html('');   // clear select comment
...
var detailCommentFn = function (id) { // by DARIS -->  show detail
	...
	$.each(dataComment['detail'], function (index, indexEntry) {
		if (indexEntry['emp_id'] == id) {
			...
			$("#span-emp").html(indexEntry['emp_name'] + " &nbsp;&nbsp;");
...
var assignTemplateQualityFn = function (structureName, data, check_disabled_first, check_disabled_second) {
	...
	htmlTemplateQuality += "<div class=\"span12\">";
	htmlTemplateQuality += "<div class=\"btn-group\">";
	htmlTemplateQuality += "<button id='btnComment1' style='border-bottom-right-radius: 0;   border-top-right-radius: 0;  min-width: 100%;' data-toggle=\"dropdown\" class=\"btn btn-success dropdown-toggle\"><span style='float:left;  margin-top: 3px;' class=\"icon-th-list\"></span><span id='span-filter1' style='float:right;'> &nbsp;&nbsp;ทั้งหมด</span> </button>";
	htmlTemplateQuality += "<ul style='min-width: 100%;' class=\"dropdown-menu\">";
	htmlTemplateQuality += "<li><a onclick='btnCommentFn(3)'>ผู้ร่วมงาน</a></li>";
	htmlTemplateQuality += "<li><a onclick='btnCommentFn(2)'>ตอบกลับแล้ว </a></li>";
	htmlTemplateQuality += "<li><a onclick='btnCommentFn(1)'>ยังไม่ได้ตอบกลับ</a></li>";
	htmlTemplateQuality += "</ul>";
	htmlTemplateQuality += "</div>";
	htmlTemplateQuality += "<div style=\"margin-left: 0px;\" class=\"btn-group\">";
	htmlTemplateQuality += "<button style='border-bottom-left-radius: 0;   border-top-left-radius: 0;  min-width: 100%;' data-toggle=\"dropdown\" id='sel-empname1' class=\"btn dropdown-toggle\"><span id='span-emp1' style='float:left;'>#4738 (ประเมินตนเอง) &nbsp;&nbsp;</span><span style='float:right;' class=\"caret\"></span></button>";
	htmlTemplateQuality += " <ul id='SelComment1' style='min-width: 100%;' class=\"dropdown-menu\">";

	//	var num=1; var emp_id = '';
	//	$.each(dataComment['detail'],function(index,indexEntry){ // gen select 
	//		if(num){num=0; emp_id =indexEntry['emp_id'];}
	//		htmlTemplateQuality+="<li onclick='detailCommentFn(\""+indexEntry['emp_id']+"\")'><a>"+indexEntry['emp_name']+"</a></li>";
	//	});

	htmlTemplateQuality += "<li><a href=\"#\">Action1</a></li>";
	htmlTemplateQuality += "<li><a href=\"#\">Action2</a></li>";
	htmlTemplateQuality += "<li><a href=\"#\">Action3</a></li>";
	htmlTemplateQuality += "</ul>";
	htmlTemplateQuality += " </div>";
	htmlTemplateQuality += "</div>";
	htmlTemplateQuality += "</div>"
	...
	htmlTemplateQuality += "<th style='text-align: center;'><b>" + $(".lt-first-line-score").val() + "</b></th>";
...
var findOneFn = function (id) {
	...
	$.ajax({
		url: restfulURL + "/" + serviceName + "/public/show/emp_result", 
		type: "get",
		dataType: "json",
		data: { "emp_result_id": id, "assessor_group_id": $("#group_id-" + id).val() },
		async: false,
		headers: { Authorization: "Bearer " + tokenID.token },
		success: function (data) {
			//// console.log(data);
...
var listDataFn = function (data) {
	...
	$.each(data['group'], function (index, indexEntry) {
		...
		if ($("#embed_appraisalType").val() == "2") {
			...
			htmlHTML += " <th style=\"width:auto;\"><b>" + $(".lt-group").val() + "</b></th>";
		...
		$.each(indexEntry['items'], function (index2, itemEntry) {
			...
			if ($("#embed_appraisalType").val() == "2") {
				...
				htmlHTML += "<input type='hidden' name='group_id-" + itemEntry['emp_result_id'] + "' id='group_id-" + itemEntry['emp_result_id'] + "'  value='" + itemEntry['group_id'] + "' >";   // by DARIS
				htmlHTML + "</td>";
				...
				htmlHTML += "	<td>" + itemEntry['group_name'] + "</td>";
		...
		htmlHTML += " </tbody>";
		htmlHTML += "  </table>";
		htmlHTML += "<input type='hidden' id='group_id'  value='' >";   // by DARIS
		htmlHTML += " </div>";
		htmlHTML += "</div>";
		htmlHTML += "</div>";
		htmlHTML += "</div>";
...
var saveAppraisalFn = function () {
	...
	updateCommentFn();   // by DARIS -->  save comment 
