...
// restfulPath
galbalDataTemp['restfulPathEmployeeSnapshot']=restfulURL + "/" + serviceName + "/public/vsm_employee";
...
var validationFn = function(data){
	...
	$.each(data['data'], function(index, indexEntry) {
		if (count == 0) {
			validate += "<font color='red'>* </font>" + indexEntry[0] + "";
		} else {
			validate += "<br><font color='red'>* </font> " + indexEntry[0] + " ";
		}
		count ++;
	});
...
var findOneFn = function(id) {
	...
	$.ajax({
		...
		headers:{Authorization:"Bearer "+tokenID.token},
		success : function(data) {	
			$("#from_emp_id").val(data['emp_snapshot_id']);
			$("#from_emp_name").val(data['emp_first_name']);
			$("#from_emp_last_name").val(data['emp_last_name']);
		}
	});
};
//--------- findOne
...
var updateFn = function () {
	var emp_snap_id = $("#from_emp_id").val();
$.ajax({
		url:galbalDataTemp['restfulPathEmployeeSnapshot']+"/"+emp_snap_id,
		type:"patch",
		dataType:"json",
		async:true,
		data : {
			"emp_first_name": $("#from_emp_name").val(),
			"emp_last_name" :  $("#from_emp_last_name").val()
		},
		headers:{Authorization:"Bearer "+tokenID.token},
		success : function(data) {
			if (data['status'] == "200") {
...
var listEmployeeSnapFn = function(data) {
	...
	$.each(data,function(index,indexEntry) {
		...
		htmlTable += "<td id=\"objectCenter\" style=\"vertical-align: middle;\"><button id="+indexEntry["emp_snapshot_id"]+" data-target=#ModalViewEmployee data-toggle='modal' data-backdrop='"+setModalPopup[0]+"' data-keyboard='"+setModalPopup[1]+"' class='btn btn-primary btn-xs view'><i class='fa fa-wrench' aria-hidden='true'></i></button></td>";
